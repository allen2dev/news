...
- 数据：通过公开 RSS（经 [rss2json](https://rss2json.com/) 代理）在浏览器端拉取，无需自建后端。个别源若无法被该服务解析会被自动跳过（国际类已用稳定可解析的源替代易失败的直连 Reuters）。

- 文章详情在应用内打开，正文经 DOMPurify 净化；完整报道仍可在新标签页打开原文。
